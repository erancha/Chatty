...
if currentUserId then
    ...
    return nil  -- No user found for connection ID
        ...
        `;
    const updatedConnectionIds = await redisClient.eval(luaScript, 2, currentConnectionId, stackName);
    if (updatedConnectionIds) console.log(JSON.stringify({ currentConnectionId, updatedConnectionIds }));
    else console.warn(`No user found for connection ID: '${currentConnectionId}' , stackName: ${stackName}`);
  } catch (error) {
    console.error(error);
